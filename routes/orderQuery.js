const getUserWithEmail = function(db, email) {
  ...
  const getOrderData = function(db, username) {
    const queryString = `
  ...
  const updateOrderStatus = function(db, dataArray) {
  ...
  const updateOrderSubmission = function(db, dataArray) {
    let array = [];
    for (let i=1; i<dataArray.length; i++) {
      ...
      array.push(db.query(queryString, values))
    }
